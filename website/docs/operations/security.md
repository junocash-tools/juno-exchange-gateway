...
- expose only the public gateway; bind the coordinator to a private exchange network
...
- keep coordinator, node RPC, scanner, ZMQ, signer socket, and databases on private networks
...
An empty credential list grants anonymous public-gateway access only on regtest. The coordinator refuses to start without at least one `plan` or `admin` credential, including on regtest.

## Secrets and keys

The gateway, coordinator, planner, scanner, node, and exchange application must never receive a seed, mnemonic, spending key, or signer share. Derive the UFVK in an isolated environment, transfer only that UFVK, and mount spending material only into the network-disabled signer. Keep signer backups encrypted and geographically separated. A UFVK is watch-only but privacy-sensitive: it reveals wallet activity and address derivation.
...
Coordinator creation requires `plan`, a wallet grant, a principal-scoped idempotency key, a prior exchange approval reference, and policy-valid outputs. The exchange must authorize before calling it. The signer socket is owner-only and never exposed through TCP.

Broadcast additionally requires a registered, credential-authorized `wallet_id`, a payload-bound idempotency key, and expected txid. The wallet binding is for authorization and audit; it does not prove that the transaction spends that wallet. Creation and broadcast controls limit retries and access; they do not replace exchange withdrawal accounting or approval.
